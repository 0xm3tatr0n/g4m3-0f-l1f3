...
function Gallery(props) {
  ...
  return (
    <div style={{ maxWidth: "100%", margin: "auto", padding: "0 16px 32px 16px" }}>
      <Row gutter={16}>
        <GalleryControl zoomLevel={zoomLevel} setZoomLevel={setZoomLevel} setGalleryLoadRange={setGalleryLoadRange} />
      </Row>
      <Row>{/* <Stats collectibles={allCollectibles} totalSupply={totalSupply} /> */}</Row>
      ...
      >
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: "16px" }}>
          <div>
            <span style={{ color: "#00d0ff" }}>Tokens available:</span> {allCollectibles?.length || 0}
          </div>
          <div>
            <span style={{ color: "#00d0ff" }}>Epochs found:</span> {Object.keys(collectiblesByEpoch).length}
          </div>
          <div>
            <span style={{ color: "#00d0ff" }}>Total minted:</span> {totalSupply?.toString() || "Loading..."}
          </div>
        </div>
      ...
      <div style={{ position: "relative" }}>
        ...
        {isLoadingGallery && (
          ...
          >
            <Spin size="large" />
            <div style={{ fontFamily: "monospace", marginTop: "8px" }}>Loading more tokens...</div>
            <Progress percent={Math.round(loadProgress)} status="active" style={{ width: "80%" }} />
        ...
        >
          ...
          >
            ...
            {allCollectibles.length === 0 && !isLoadingGallery && (
              ...
              >
                <div style={{ color: "yellow", fontSize: "16px" }}>No collectibles found to display</div>
                <div style={{ fontSize: "14px", color: "#aaa", maxWidth: "600px", margin: "0 auto" }}>
                  To populate the gallery, run <span style={{background: "#333", padding: "2px 6px", borderRadius: "4px", fontWeight: "bold"}}>yarn extract-tokens</span> in your terminal. 
                  This will extract all minted tokens from the blockchain to your local assets directory.
                </div>
                <div style={{ fontSize: "14px", color: "#aaa", maxWidth: "600px", margin: "0 auto" }}>
                  If you haven't minted any tokens yet, use <span style={{background: "#333", padding: "2px 6px", borderRadius: "4px", fontWeight: "bold"}}>yarn mint-tokens 10</span> to mint 10 new tokens.
                </div>

...
const main = async () => {
  ...
  const G0lLibrary = await deploy('Libraries/G0l.sol');
  await G0lLibrary.deployed();

  const BitOpsLibrary = await deploy('Libraries/BitOps.sol');
  await BitOpsLibrary.deployed();
  ...
  await yourCollectible.link('G0l', G0lLibrary.address);
  await yourCollectible.link('BitOps', BitOpsLibrary.address);
  ...
  */
  ...
  const MINTS_10 = 8; // how many times to mintMany, max 10 per transaction
